...
abstract class GenICode extends SubComponent {
  ...
  class ICodePhase(prev: Phase) extends StdPhase(prev) {
    ...
    private def genPrint(msg : String, pos: Position) : Tree = { 
      ...
      val liter0 = Literal(Constant(extractEssentialPath(pos.focus.toString)))
      liter0.setType(typeOf[String])
      
      val liter1 = Literal(Constant(msg)) 
      liter1.setType(typeOf[String])              

      /*val sym = rootMirror.getModuleIfDefined("java.lang.System")            
      val printStream = rootMirror.getRequiredClass("java.io.PrintStream")
      val receiver = definitions.getMember(sym, newTermName("out")) 
      val newPrint = global.gen.mkMethodCall(receiver, newTermName("print"), List(liter))  
      val Apply(fun, _) = newPrint
      fun.setSymbol(definitions.PredefModule.info.decl(newTermName("print")))
      fun.setType(definitions.PredefModule.info.decl(newTermName("print")).tpe)
      val Select(qual, _) = fun
      qual.setType(receiver.tpe) 
      newPrint.setType(typeOf[Unit])
      newPrint*/
      
      val sym = rootMirror.staticModule("ScalaTrace")
      val receiver = definitions.getMember(sym, newTermName("logger"))
      val newPrint = global.gen.mkMethodCall(receiver, newTermName("log"), List(liter0, liter1))
      val Apply(fun, _) = newPrint
      fun.setSymbol(receiver.info.decl(newTermName("log")))
      fun.setType(receiver.info.decl(newTermName("log")).tpe)
      val Select(qual, _) = fun
      qual.setType(receiver.tpe)
      newPrint.setType(typeOf[Unit])
      newPrint
    ...
    def emitAndGenPrint(ctx: Context, ins: Instruction, emitMode: String, argPos: Position = NoPosition) {
      
      val pos = emitMode match {
        case "emitDefault" => if (ctx.bb.instructionList.isEmpty) NoPosition else ctx.bb.instructionList.head.pos   
        case "emit" => argPos
        case "emitOnly" => 
          if (ins.pos == NoPosition) 
            if (ctx.bb.instructionList.isEmpty) NoPosition 
            else ctx.bb.instructionList.head.pos 
          else ins.pos
        case "closeWithDefault" => if (ctx.bb.instructionList.isEmpty) NoPosition else ctx.bb.instructionList.head.pos
        case "closeWith" => argPos
        case "emitSeq" => ins.pos
      }
      ...
      ins match {
         ...
         */
        case PJUMP(label: Label) =>         
        case PCJUMP(success: Label, failure: Label, cond: TestOp, kind: TypeKind) => 
        case PCZJUMP(success: Label, failure: Label, cond: TestOp, kind: TypeKind) => 
        
        /*
         * override def consumed = 0
         * override def produced = 1
         */
        case THIS(clasz: Symbol) => 
          genStraight(ctx, "#2 @this", pos)
        ...
        case CONSTANT(constant: Constant) =>
          ...
          genStraight(ctx, "#2 *", pos)

        /*
         * override def consumed = 2
         * override def produced = 1
         */
        case LOAD_ARRAY_ITEM(kind: TypeKind) =>
          genStraight(ctx, "#1 @index", pos)
          genStraight(ctx, "#1 @array", pos)
          genStraight(ctx, "#3 @index @array @array[@index]", pos)
          genStraight(ctx, "#2 @array[@index]", pos) 

        /*
         * override def consumed = 0
         * override def produced = 1       
         */
        case LOAD_LOCAL(local: Local) =>
          genStraight(ctx, "#2 " + local.sym.fullName, pos)

        /*
         * override def consumed = if (isStatic) 0 else 1
         * override def produced = 1
         */
        case LOAD_FIELD(field: Symbol, isStatic: Boolean) =>
          if(isStatic)
            genStraight(ctx, "#2 " + field.fullName, pos)
          else {
            genStraight(ctx, "#1 this", pos)
            genStraight(ctx, "#3 this " + field.fullName, pos)
            genStraight(ctx, "#2 " + field.fullName, pos)
          }
         ...
         * override def produced = 1
         */
        case LOAD_MODULE(module: Symbol) =>
          genStraight(ctx, "#2 " + module.fullName, module.pos)
          genStraight(ctx, "#2 " + module.fullName, module.pos)
          genStraight(ctx, "##03", module.pos)

        /*
         * override def consumed = 3
         * override def produced = 0
         */
        case STORE_ARRAY_ITEM(kind: TypeKind) =>
          genStraight(ctx, "#1 @value", pos)
          genStraight(ctx, "#1 @index", pos)
          genStraight(ctx, "#1 @array", pos)
          genStraight(ctx, "#3 @value @array[@index]", pos)

        /*
         * override def consumed = 1
         * override def produced = 0
         */
        case STORE_LOCAL(local: Local) =>
          genStraight(ctx, "#1 " + local.sym.fullName, pos)

        /*
         * override def consumed = if(isStatic) 1 else 2
         * override def produced = 0
         */
        case STORE_FIELD(field: Symbol, isStatic: Boolean) =>
          if(isStatic)
            genStraight(ctx, "#1 " + field.fullName, pos)
          else {
            genStraight(ctx, "#1 " + field.fullName, pos)
            genStraight(ctx, "#1 this", pos)
          }
         ...
         * override def produced = 1
         */
        case CALL_PRIMITIVE(primitive: Primitive) =>
          def genTwoOperandPrimitive() = {
            genStraight(ctx, "#1 @operand1", pos)
            genStraight(ctx, "#1 @operand2", pos)
            genStraight(ctx, "#3 @operand1 @operand2 @result", pos)
            genStraight(ctx, "#2 @result", pos)
          }
          
         primitive match {
            case Negation(_)       => 
            case Test(_,_, true)   => //why only one operand?
            case Test(_,_, false)  => genTwoOperandPrimitive()
            case Comparison(_,_)   => genTwoOperandPrimitive()
              
            case Arithmetic(NOT,_) => 
            case Arithmetic(_,_)   => genTwoOperandPrimitive() 
            case Logical(_,_)      => genTwoOperandPrimitive()
            case Shift(_,_)        => genTwoOperandPrimitive()
            case Conversion(_,_)   => 
            case ArrayLength(_)    => 
            case StringConcat(_)   => genTwoOperandPrimitive()
            case StartConcat       => genStraight(ctx, "#2 @StartConcat", pos)
            case EndConcat         => 
         }
         genStraight(ctx, "##04", pos)

        case CALL_METHOD(method: Symbol, style: InvokeStyle) => 
          def params = method.info.paramTypes
          def consumesInstance = style match {
            case Static(false)  => 0
            case _              => 1
          }
    
          def consumed = params.length + consumesInstance
          val producedList = toTypeKind(method.info.resultType) match {
            case UNIT                      => Nil
            case _ if method.isConstructor => Nil
            case kind                      => kind :: Nil
          }
          def produced = producedList.size
          
          if(method.isJava) {
            for(i <- (1 to ins.consumed)) {
              genStraight(ctx, "#1 @javaParam" + i, pos)
            }
            
            val params = 
              if(ins.consumed > 0) 
                (1 to ins.consumed).toList.map { x => "@javaParam" + x }.reduce { (a, b) => a + " " + b }           
              else 
                  ""
            
            for(i <- (1 to ins.produced)) {
              if(ins.consumed > 0)
                genStraight(ctx, "#3 " + params + " @javaResult" + i, pos)
              genStraight(ctx, "#2 @javaResult" + i, pos)
            }
            genStraight(ctx, "##06", method.pos)
          }
          
          if(!method.isJava && consumesInstance == 1) {
            genStraight(ctx, "#1 @this @", pos)
          }
         ...
         */
        case INVOKE_DYNAMIC(poolEntry: Int) => 

        /*
         * override def consumed = 1
         * override def produced = 1
         */
        case BOX(boxType: TypeKind) => 
          
        /*
         * override def consumed = 1
         * override def produced = 1
         */
        case UNBOX(boxType: TypeKind) => 

        /*
         * override def consumed = 0
         * override def produced = 1
         */
        case NEW(kind: REFERENCE) =>
          genStraight(ctx, "#2 " + kind.cls.fullName, pos)

        /*
         * override def consumed = dims
         * override def produced = 1
         */
        case CREATE_ARRAY(elem: TypeKind, dims: Int) =>
          for(x <- 1 to dims) {
            genStraight(ctx, "#1 @CREATE_ARRAY[" + x + "]", pos)
          }
          val initials = 
            if(dims > 0)
              (1 to dims).map { x => "@CREATE_ARRAY[" + x + "]" }.reduce { (a, b) => a + " " + b }
              else ""
          if(dims > 0)
            genStraight(ctx, "#3 " + initials + " @CREATE_ARRAY", pos)
          genStraight(ctx, "#2 @CREATE_ARRAY", pos)

        /*
         * override def consumed = 1
         * override def produced = 1
         */
        case IS_INSTANCE(typ: TypeKind) => 

        /*
         * override def consumed = 1
         * override def produced = 1
         */
        case CHECK_CAST(typ: TypeKind) => 

        /*     
         * override def consumed = 1
         * override def produced = 0
         */
        case SWITCH(tags: List[List[Int]], labels: List[BasicBlock]) =>
          genStraight(ctx, "#1 @SWITCH", pos)

        /*
         * override def consumed = 0
         * override def produced = 0
         */
        case JUMP(whereto: BasicBlock) => 

        /*
         * override def consumed = 2   
         * override def produced = 0
         */
        case CJUMP(successBlock: BasicBlock,
             failureBlock: BasicBlock,
             cond: TestOp, kind: TypeKind) =>    
               genStraight(ctx, "#1 @CJUMP1", pos)
               genStraight(ctx, "#1 @CJUMP2", pos)

        /*
         * override def consumed = 1
         * override def produced = 0
         */
        case CZJUMP(successBlock: BasicBlock,
                          failureBlock: BasicBlock,
                          cond: TestOp, kind: TypeKind) =>
                            genStraight(ctx, "#1 @CZJUMP", pos)
         ...
         * override def produced = 0
         */
        case RETURN(kind: TypeKind) => 
          genStraight(ctx, "##01", pos) 

        /*
         * override def consumed = 1
         * override def produced = 0
         */
        case THROW(clasz: Symbol) =>
          genStraight(ctx, "#1 @THROW", pos)

        /*
         * override def consumed = 1
         * override def produced = 0
         */
        case DROP (typ: TypeKind) =>
          genStraight(ctx, "#1 @DROP", pos)

        /*
         * override def consumed = 1
         * override def produced = 2
         */
        case DUP (typ: TypeKind) =>
          genStraight(ctx, "#1 @DUP", pos)
          genStraight(ctx, "#3 @DUP @DUP1", pos)
          genStraight(ctx, "#2 @DUP1", pos)
          genStraight(ctx, "#3 @DUP @DUP2", pos)
          genStraight(ctx, "#2 @DUP2", pos)
         ...
         * override def produced = 0
         */
        case SCOPE_ENTER(lv: Local) => 
          
        /*
         * override def consumed = 0
         * override def produced = 0
         */
        case SCOPE_EXIT(lv: Local) => 

        /*
         * override def consumed = sys.error("LOAD_EXCEPTION does clean the whole stack, no idea how many things it consumes!")
         * override def produced = 1
         */
        case LOAD_EXCEPTION(clasz: Symbol) => 
          genStraight(ctx, "##08", pos)
          genStraight(ctx, "#2 @ex", pos)
      }
      ...
      ins match {
        case CALL_METHOD(method: Symbol, style: InvokeStyle) =>   
          if(method.isJava && !method.fullName.contains("java.io.PrintStream.print"))  
            genStraight(ctx, "##07", pos)
        case CALL_PRIMITIVE(primitive: Primitive) =>
          genStraight(ctx, "##05", pos)
        case _ => 
      }
    }
    ...
    def gen(tree: Tree, ctx: Context): Context = tree match {
      ...
      case DefDef(mods, name, tparams, vparamss, tpt, rhs) =>
        ...
        genStraight(ctx1, "##00 " + m.symbol.fullName + " class: " + m.symbol.owner, tree.pos)
        ...
        if (!m.isAbstractMethod && !m.native) {
          ...
          rhs match {
            ...
            case _ => if (ctx1.bb.isEmpty) {
              if(!ctx1.bb.closed) {
                emitAndGenPrint(ctx1, RETURN(m.returnType), "closeWith", rhs.pos)
                ctx1.bb.close()
              }
            }
              ...
              if(!ctx1.bb.closed) {
              emitAndGenPrint(ctx1, RETURN(m.returnType), "closeWithDefault")
              ctx1.bb.close()
              }
    ...
    private def genStat(tree: Tree, ctx: Context): Context = tree match {
      case Assign(lhs @ Select(_, _), rhs) =>
        ...
        emitAndGenPrint(ctx1, STORE_FIELD(lhs.symbol, isStatic), "emit", tree.pos)
        ctx1
      ...
      case Assign(lhs, rhs) =>
        ...
        emitAndGenPrint(ctx1, STORE_LOCAL(l), "emit", tree.pos)
        ctx1
    ...
    private def genThrow(expr: Tree, ctx: Context): (Context, TypeKind) = {
      ...
      emitAndGenPrint(ctx1, THROW(expr.tpe.typeSymbol), "emit", expr.pos)
    ...
    private def genArithmeticOp(tree: Tree, ctx: Context, code: Int): (Context, TypeKind) = {
      ...
      args match {
        ...
        case Nil =>
          ...
          code match {
            ...
            case scalaPrimitives.NEG =>
              ...
              emitAndGenPrint(ctx1, CALL_PRIMITIVE(Negation(resKind)), "emit", larg.pos)
            case scalaPrimitives.NOT =>
              //ctx1.bb.emit(CALL_PRIMITIVE(Arithmetic(NOT, resKind)), larg.pos)
              emitAndGenPrint(ctx1, CALL_PRIMITIVE(Arithmetic(NOT, resKind)), "emit", larg.pos)
            case _ =>
        ...
        case rarg :: Nil =>
          ...
          emitAndGenPrint(ctx1, CALL_PRIMITIVE(primitiveOp), "emit", tree.pos)
    ...
    private def genArrayOp(tree: Tree, ctx: Context, code: Int, expectedType: TypeKind): (Context, TypeKind) = {
      ...
      if (scalaPrimitives.isArrayGet(code)) {
        ...
        emitAndGenPrint(ctx1, LOAD_ARRAY_ITEM(elementType), "emit", tree.pos)
        ...
        emitAndGenPrint(ctx1, STORE_ARRAY_ITEM(elementType), "emit", tree.pos)
      }
      else {
        generatedType = INT
        //ctx1.bb.emit(CALL_PRIMITIVE(ArrayLength(elementType)), tree.pos)
        emitAndGenPrint(ctx1, CALL_PRIMITIVE(ArrayLength(elementType)), "emit", tree.pos)
      }
    ...
    private def genSynchronized(tree: Apply, ctx: Context, expectedType: TypeKind): (Context, TypeKind) = {
      ...
      emitAndGenPrint(ctx1, DUP(ObjectReference), "emitSeq")
      emitAndGenPrint(ctx1, STORE_LOCAL(monitor), "emitSeq")
      emitAndGenPrint(ctx1, MONITOR_ENTER() setPos tree.pos, "emitSeq")
      /*ctx1.bb.emit(Seq(
        DUP(ObjectReference),
        STORE_LOCAL(monitor),
      ...
      ctx1 = ctx1.Try(
        bodyCtx => {
          ...
          if (hasResult) {
            ...
            emitAndGenPrint(ctx2, STORE_LOCAL(monitorResult), "emitDefault")
          }
          emitAndGenPrint(ctx2, LOAD_LOCAL(monitor), "emitSeq")
          emitAndGenPrint(ctx2, MONITOR_EXIT() setPos tree.pos, "emitSeq")
          /*ctx2.bb.emit(Seq(
            LOAD_LOCAL(monitor),
          ...
          (ThrowableClass, expectedType, exhCtx => {
            emitAndGenPrint(exhCtx, LOAD_LOCAL(monitor), "emitSeq")
            emitAndGenPrint(exhCtx, MONITOR_EXIT() setPos tree.pos, "emitSeq")
            emitAndGenPrint(exhCtx, THROW(ThrowableClass), "emitSeq")
            /*exhCtx.bb.emit(Seq(
              LOAD_LOCAL(monitor),
              MONITOR_EXIT() setPos tree.pos,
      ...
      if (hasResult) {
        ...
        emitAndGenPrint(ctx1, LOAD_LOCAL(monitorResult), "emitDefault")
      }
      (ctx1, expectedType)
    }
    ...
    private def genLoadIf(tree: If, ctx: Context, expectedType: TypeKind): (Context, TypeKind) = {
      ...
      if(!thenCtx.bb.closed) {
        emitAndGenPrint(thenCtx, JUMP(contCtx.bb), "closeWithDefault")
        thenCtx.bb.close()
      }
      //thenCtx.bb.closeWith(JUMP(contCtx.bb))
      if(!elseCtx.bb.closed) {
        emitAndGenPrint(elseCtx, if (elsep == EmptyTree) JUMP(contCtx.bb)
          else JUMP(contCtx.bb) setPos tree.pos, "closeWithDefault")
          elseCtx.bb.close()
      }
        ...
        for (CaseDef(pat, _, body) <- catches.reverse) yield {
          def genWildcardHandler(sym: Symbol): (Symbol, TypeKind, Context => Context) =
            (sym, kind, ctx => {
              ...
              emitAndGenPrint(ctx, DROP(REFERENCE(sym)), "emitDefault")
            ...
            case Bind(_, _)                       =>
              ...
              (pat.symbol.tpe.typeSymbol, kind, {
                ctx: Context =>
                  ...
                  emitAndGenPrint(ctx, STORE_LOCAL(exception), "emit", pat.pos)
                  genLoad(body, ctx, kind)
              })
          }
        }
    ...
    private def genPrimitiveOp(tree: Apply, ctx: Context, expectedType: TypeKind): (Context, TypeKind) = {
      ...
      if (scalaPrimitives.isArithmeticOp(code))
        ...
        emitAndGenPrint(trueCtx, CONSTANT(Constant(true)) setPos tree.pos, "emitOnly")
        emitAndGenPrint(trueCtx, JUMP(afterCtx.bb), "emitOnly")
        trueCtx.bb.close()
        /*trueCtx.bb.emitOnly(
          CONSTANT(Constant(true)) setPos tree.pos,
          JUMP(afterCtx.bb)
        )*/
        emitAndGenPrint(falseCtx, CONSTANT(Constant(false)) setPos tree.pos, "emitOnly")
        emitAndGenPrint(falseCtx, JUMP(afterCtx.bb), "emitOnly")
        falseCtx.bb.close()
      ...
      val resCtx: Context = tree match {
        case LabelDef(name, params, rhs) =>
          def genLoadLabelDef = {
            ...
            if(!ctx.bb.closed) {
              emitAndGenPrint(ctx, JUMP(ctx1.bb), "closeWith", tree.pos)
              ctx.bb.close()
            }
          ...
          def genLoadValDef =
            if (name == nme.THIS) {
              ...
              if (rhs == EmptyTree) {
                ...
                emitAndGenPrint(ctx, getZeroOf(local.kind), "emitDefault")
              }
              ...
              emitAndGenPrint(ctx1, STORE_LOCAL(local), "emit", tree.pos)
              ctx1.scope.add(local)
              //ctx1.bb.emit(SCOPE_ENTER(local))
              emitAndGenPrint(ctx1, SCOPE_ENTER(local), "emitDefault")
              generatedType = UNIT
              ctx1
            }
          genLoadValDef
        ...
        case Return(expr) =>
          def genLoadReturn = {
            ...
            val saved        = savingCleanups(ctx1) {
              ...
              ctx1.cleanups foreach {
                case MonitorRelease(m) =>
                  ...
                  emitAndGenPrint(ctx1, LOAD_LOCAL(m), "emitSeq")
                  emitAndGenPrint(ctx1, MONITOR_EXIT(), "emitSeq")
                ...
                case Finalizer(f, finalizerCtx) =>
                  ...
                  if (returnedKind != UNIT && mayCleanStack(f)) {
                    ...
                    emitAndGenPrint(ctx1, STORE_LOCAL(tmp), "emitDefault")
                  ...
                  if(!ctx1.bb.closed) {
                    emitAndGenPrint(ctx1, JUMP(fctx.bb), "closeWithDefault")
                    ctx1.bb.close()
                  }
            ...
            if (saved) {
              ...
              emitAndGenPrint(ctx1, LOAD_LOCAL(tmp), "emitDefault")
              //ctx1.bb.emit(LOAD_LOCAL(tmp))
            }
            adapt(returnedKind, ctx1.method.returnType, ctx1, tree.pos)
            emitAndGenPrint(ctx1, RETURN(ctx.method.returnType), "emit", tree.pos)
        ...
        case Apply(TypeApply(fun, targs), _) =>
          def genLoadApply1 = {
            ...
            if (l.isValueType && r.isValueType)
              ...
              emitAndGenPrint(ctx1, DROP(l), "emit", fun.pos)
              //ctx1.bb.emit(DROP(l), fun.pos)
              if (cast) {
                emitAndGenPrint(ctx1, NEW(REFERENCE(definitions.ClassCastExceptionClass)), "emitSeq")
                emitAndGenPrint(ctx1, DUP(ObjectReference), "emitSeq")
                emitAndGenPrint(ctx1, THROW(definitions.ClassCastExceptionClass), "emitSeq")
                /*ctx1.bb.emit(Seq(
                ...
                emitAndGenPrint(ctx1, CONSTANT(Constant(false)), "emitDefault")
              ...
              emitAndGenPrint(ctx, IS_INSTANCE(REFERENCE(definitions.boxedClass(r.toType.typeSymbol))), "emitDefault")
        ...
        case Apply(fun @ Select(Super(_, mix), _), args) =>
          def genLoadApply2 = {
            ...
            emitAndGenPrint(ctx, THIS(ctx.clazz.symbol), "emit", tree.pos)
            //ctx.bb.emit(THIS(ctx.clazz.symbol), tree.pos)
            val ctx1 = genLoadArguments(args, fun.symbol.info.paramTypes, ctx)
            emitAndGenPrint(ctx1, CALL_METHOD(fun.symbol, invokeStyle), "emit", tree.pos)
          ...
          def genLoadApply3 = {
            ...
            generatedType match {
              case arr @ ARRAY(elem) =>
                ...
                emitAndGenPrint(ctx1, CREATE_ARRAY(elemKind, args.length), "emit", tree.pos)
              ...
              case rt @ REFERENCE(cls) =>
                ...
                val nw = NEW(rt)
                emitAndGenPrint(ctx, nw, "emit", tree.pos)
                //ctx.bb.emit(nw, tree.pos)
                emitAndGenPrint(ctx, DUP(generatedType), "emitDefault")
                //ctx.bb.emit(DUP(generatedType))
                val ctx1 = genLoadArguments(args, ctor.info.paramTypes, ctx)

                val init = CALL_METHOD(ctor, Static(onInstance = true))
                nw.init = init
                emitAndGenPrint(ctx1, init, "emit", tree.pos)
        ...
        case Apply(fun @ _, List(expr)) if currentRun.runDefinitions.isBox(fun.symbol) =>
          def genLoadApply4 = {
            ...
            if (settings.Xdce) {
              ...
              emitAndGenPrint(ctx1, STORE_LOCAL(loc1), "emitDefault")
              //ctx1.bb.emit(STORE_LOCAL(loc1))
              emitAndGenPrint(ctx1, LOAD_LOCAL(loc1), "emitDefault")
              //ctx1.bb.emit(LOAD_LOCAL(loc1))
            }
            emitAndGenPrint(ctx1, BOX(nativeKind), "emit", expr.pos)
        ...
        case Apply(fun @ _, List(expr)) if (currentRun.runDefinitions.isUnbox(fun.symbol)) =>
          ...
          emitAndGenPrint(ctx1, UNBOX(boxType), "emit", expr.pos)
        ...
        case app @ Apply(fun, args) =>
          def genLoadApply6 = {
            ...
            if (sym.isLabel) {  // jump to a label
              ...
              emitAndGenPrint(ctx1, if (label.anchored) JUMP(label.block) else PJUMP(label), "emitOnly")
              ctx1.bb.close()
              ...
              emitAndGenPrint(ctx1, cm, "emit", tree.pos)
        ...
        case Select(qualifier, selector) =>
          def genLoadSelect = {
            ...
            if (sym.isModule) {
              ...
              emitAndGenPrint(ctx1, LOAD_FIELD(sym, isStatic) setHostClass hostClass, "emit", tree.pos)
            ...
            if (!sym.hasPackageFlag) {
              if (sym.isModule) {
                ...
                ctx.method.lookupLocal(sym) match {
                  case Some(l) =>
                    emitAndGenPrint(ctx, LOAD_LOCAL(l), "emit", tree.pos)
        ...
        case Literal(value) =>
          def genLoadLiteral = {
            if (value.tag != UnitTag) (value.tag, expectedType) match {
              case (IntTag, LONG) =>
                emitAndGenPrint(ctx, CONSTANT(Constant(value.longValue)), "emit", tree.pos)
                //ctx.bb.emit(CONSTANT(Constant(value.longValue)), tree.pos)
                generatedType = LONG
              case (FloatTag, DOUBLE) =>
                emitAndGenPrint(ctx, CONSTANT(Constant(value.doubleValue)), "emit", tree.pos)
                //ctx.bb.emit(CONSTANT(Constant(value.doubleValue)), tree.pos)
                generatedType = DOUBLE
              case (NullTag, _) =>
                emitAndGenPrint(ctx, CONSTANT(value), "emit", tree.pos)
        ...
        case ArrayValue(tpt @ TypeTree(), _elems) =>
          def genLoadArrayValue = {
            ...
            emitAndGenPrint(ctx1, CONSTANT(new Constant(elems.length)), "emit", tree.pos)
            //ctx1.bb.emit(CONSTANT(new Constant(elems.length)), tree.pos)
            emitAndGenPrint(ctx1, CREATE_ARRAY(elmKind, 1), "emitDefault")
            //ctx1.bb.emit(CREATE_ARRAY(elmKind, 1))
            // inline array literals
            var i = 0
            while (i < elems.length) {
              emitAndGenPrint(ctx1, DUP(generatedType), "emit", tree.pos)
              //ctx1.bb.emit(DUP(generatedType), tree.pos)
              emitAndGenPrint(ctx1, CONSTANT(new Constant(i)), "emitDefault")
              //ctx1.bb.emit(CONSTANT(new Constant(i)))
              ctx1 = genLoad(elems(i), ctx1, elmKind)
              emitAndGenPrint(ctx1, STORE_ARRAY_ITEM(elmKind), "emitDefault")
        ...
        case Match(selector, cases) =>
          def genLoadMatch = {
            ...
            for (caze @ CaseDef(pat, guard, body) <- cases) {      
              ...
              if(!caseCtx.bb.closed){
                emitAndGenPrint(caseCtx, JUMP(afterCtx.bb) setPos caze.pos, "closeWithDefault")
                caseCtx.bb.close()
              }
              //caseCtx.bb.closeWith(JUMP(afterCtx.bb) setPos caze.pos)
            }
            afterCtxReachable ||= (default == afterCtx)
            emitAndGenPrint(ctx1, SWITCH(tags.reverse map (x => List(x)), (default :: targets).reverse) setPos tree.pos, 
                "emitOnly")
            ctx1.bb.close()
            /*ctx1.bb.emitOnly(
        ...
        case EmptyTree =>
          if (expectedType != UNIT)
            emitAndGenPrint(ctx, getZeroOf(expectedType), "emitDefault")
    ...
    private def adaptNullRef(from: TypeKind, to: TypeKind, ctx: Context, pos: Position) {
      ...
      if (from == NullReference && to != UNIT && to != ObjectReference && to != AnyRefReference) {
        ...
        emitAndGenPrint(ctx, DROP(from), "emit", pos)
        //ctx.bb.emit(DROP(from), pos)
        emitAndGenPrint(ctx, CONSTANT(Constant(null)), "emit", pos)
    ...
    private def adapt(from: TypeKind, to: TypeKind, ctx: Context, pos: Position) {
      ...
      def coerce(from: TypeKind, to: TypeKind) = { 
        emitAndGenPrint(ctx, CALL_PRIMITIVE(Conversion(from, to)), "emit", pos)
      ...
      (from, to) match {
        ...
        case (NothingReference, _) =>
          emitAndGenPrint(ctx, THROW(ThrowableClass), "emitDefault")
        ...
        case (NullReference, REFERENCE(_)) =>
          ...
          emitAndGenPrint(ctx, DROP(from), "emitSeq")
          emitAndGenPrint(ctx, CONSTANT(Constant(null)), "emitSeq")
    ...
    private def genLoadLabelArguments(args: List[Tree], label: Label, ctx: Context): Context = {
      ...
      stores.reverse.foreach { ins => emitAndGenPrint(ctx1, ins, "emitSeq") }
    ...
    private def genLoadModule(ctx: Context, tree: Tree): Context = {
      ...
      emitAndGenPrint(ctx, LOAD_MODULE(sym), "emit", tree.pos)
    ...
    def genConversion(from: TypeKind, to: TypeKind, ctx: Context, cast: Boolean) = {
      if (cast)
        emitAndGenPrint(ctx, CALL_PRIMITIVE(Conversion(from, to)), "emitDefault")
        //ctx.bb.emit(CALL_PRIMITIVE(Conversion(from, to)))
      else {
        emitAndGenPrint(ctx, DROP(from), "emitDefault")
        //ctx.bb.emit(DROP(from))
        emitAndGenPrint(ctx, CONSTANT(Constant(from == to)), "emitDefault")
        //ctx.bb.emit(CONSTANT(Constant(from == to)))
      }
    }

    def genCast(from: TypeKind, to: TypeKind, ctx: Context, cast: Boolean) =
      emitAndGenPrint(ctx, if (cast) CHECK_CAST(to) else IS_INSTANCE(to), "emitDefault")
    ...
    def genCoercion(tree: Tree, ctx: Context, code: Int) = {
      import scalaPrimitives._
      val ins = (code: @switch) match {
        case B2B => null
        case B2C => CALL_PRIMITIVE(Conversion(BYTE, CHAR))
        case B2S => CALL_PRIMITIVE(Conversion(BYTE, SHORT))
        case B2I => CALL_PRIMITIVE(Conversion(BYTE, INT))
        case B2L => CALL_PRIMITIVE(Conversion(BYTE, LONG))
        case B2F => CALL_PRIMITIVE(Conversion(BYTE, FLOAT))
        case B2D => CALL_PRIMITIVE(Conversion(BYTE, DOUBLE))

        case S2B => CALL_PRIMITIVE(Conversion(SHORT, BYTE))
        case S2S => null
        case S2C => CALL_PRIMITIVE(Conversion(SHORT, CHAR))
        case S2I => CALL_PRIMITIVE(Conversion(SHORT, INT))
        case S2L => CALL_PRIMITIVE(Conversion(SHORT, LONG))
        case S2F => CALL_PRIMITIVE(Conversion(SHORT, FLOAT))
        case S2D => CALL_PRIMITIVE(Conversion(SHORT, DOUBLE))

        case C2B => CALL_PRIMITIVE(Conversion(CHAR, BYTE))
        case C2S => CALL_PRIMITIVE(Conversion(CHAR, SHORT))
        case C2C => null
        case C2I => CALL_PRIMITIVE(Conversion(CHAR, INT))
        case C2L => CALL_PRIMITIVE(Conversion(CHAR, LONG))
        case C2F => CALL_PRIMITIVE(Conversion(CHAR, FLOAT))
        case C2D => CALL_PRIMITIVE(Conversion(CHAR, DOUBLE))

        case I2B => CALL_PRIMITIVE(Conversion(INT, BYTE))
        case I2S => CALL_PRIMITIVE(Conversion(INT, SHORT))
        case I2C => CALL_PRIMITIVE(Conversion(INT, CHAR))
        case I2I => null
        case I2L => CALL_PRIMITIVE(Conversion(INT, LONG))
        case I2F => CALL_PRIMITIVE(Conversion(INT, FLOAT))
        case I2D => CALL_PRIMITIVE(Conversion(INT, DOUBLE))

        case L2B => CALL_PRIMITIVE(Conversion(LONG, BYTE))
        case L2S => CALL_PRIMITIVE(Conversion(LONG, SHORT))
        case L2C => CALL_PRIMITIVE(Conversion(LONG, CHAR))
        case L2I => CALL_PRIMITIVE(Conversion(LONG, INT))
        case L2L => null
        case L2F => CALL_PRIMITIVE(Conversion(LONG, FLOAT))
        case L2D => CALL_PRIMITIVE(Conversion(LONG, DOUBLE))

        case F2B => CALL_PRIMITIVE(Conversion(FLOAT, BYTE))
        case F2S => CALL_PRIMITIVE(Conversion(FLOAT, SHORT))
        case F2C => CALL_PRIMITIVE(Conversion(FLOAT, CHAR))
        case F2I => CALL_PRIMITIVE(Conversion(FLOAT, INT))
        case F2L => CALL_PRIMITIVE(Conversion(FLOAT, LONG))
        case F2F => null
        case F2D => CALL_PRIMITIVE(Conversion(FLOAT, DOUBLE))

        case D2B => CALL_PRIMITIVE(Conversion(DOUBLE, BYTE))
        case D2S => CALL_PRIMITIVE(Conversion(DOUBLE, SHORT))
        case D2C => CALL_PRIMITIVE(Conversion(DOUBLE, CHAR))
        case D2I => CALL_PRIMITIVE(Conversion(DOUBLE, INT))
        case D2L => CALL_PRIMITIVE(Conversion(DOUBLE, LONG))
        case D2F => CALL_PRIMITIVE(Conversion(DOUBLE, FLOAT))
        case D2D => null

        case _ => abort("Unknown coercion primitive: " + code)
      } 
      if(ins != null)
        emitAndGenPrint(ctx, ins, "emit", tree.pos)
    }
    ...
    def genScalaHash(tree: Tree, ctx: Context): Context = {
      val hashMethod = {
        emitAndGenPrint(ctx, LOAD_MODULE(ScalaRunTimeModule), "emitDefault")
      ...
      emitAndGenPrint(ctx1, CALL_METHOD(hashMethod, Static(onInstance = false)), "emitDefault")
                        ...
                        elseCtx: Context): Boolean =
    {
      ...
      def genComparisonOp(l: Tree, r: Tree, code: Int): Boolean = {
        ...
        if (isReferenceEqualityOp(code) && nonNullSide != null) {
          ...
          emitAndGenPrint(ctx1, CZJUMP(thenCtx.bb, elseCtx.bb, op, ObjectReference), "emitOnly")
          ctx1.bb.close()
          /*ctx1.bb.emitOnly(
          ...
          val branchesReachable = !ctx1.bb.ignore
          emitAndGenPrint(ctx1, CJUMP(thenCtx.bb, elseCtx.bb, op, kind) setPos r.pos, "emitOnly")
          ctx1.bb.close()
          /*ctx1.bb.emitOnly(
      ...
      def default(): Boolean = {
        ...
        val branchesReachable = !ctx1.bb.ignore
        if(!ctx1.bb.closed) {
          emitAndGenPrint(ctx1, CZJUMP(thenCtx.bb, elseCtx.bb, NE, BOOL) setPos tree.pos, "closeWithDefault")
          ctx1.bb.close()
        }
    ...
    def genEqEqPrimitive(l: Tree, r: Tree, ctx: Context)(thenCtx: Context, elseCtx: Context): Boolean = {
      ...
      if (mustUseAnyComparator) {
        ...
        emitAndGenPrint(ctx2, CALL_METHOD(equalsMethod, if (settings.optimise) Dynamic else Static(onInstance = false)),
            "emitOnly")
        emitAndGenPrint(ctx2, CZJUMP(thenCtx.bb, elseCtx.bb, NE, BOOL), "emitOnly")
        ctx2.bb.close()
        /*ctx2.bb.emitOnly(
        ...
        if (isNull(l)) {
          ...
          val branchesReachable = !ctx1.bb.ignore
          emitAndGenPrint(ctx1, CZJUMP(thenCtx.bb, elseCtx.bb, EQ, ObjectReference), "emitOnly")  
          ctx1.bb.close()
          ...
          emitAndGenPrint(ctx1, CALL_METHOD(Object_equals, Dynamic), "emitOnly")
          emitAndGenPrint(ctx1, CZJUMP(thenCtx.bb, elseCtx.bb, NE, BOOL), "emitOnly")
          ctx1.bb.close()
          /*ctx1.bb emitOnly(
          ...
          emitAndGenPrint(ctx1, STORE_LOCAL(eqEqTempLocal) setPos l.pos, "emitOnly")
          emitAndGenPrint(ctx1, DUP(ObjectReference), "emitOnly")
          emitAndGenPrint(ctx1, CZJUMP(nullCtx.bb, nonNullCtx.bb, EQ, ObjectReference), "emitOnly")
          ctx1.bb.close()
          /*ctx1.bb.emitOnly(
          ...
          emitAndGenPrint(nullCtx, DROP(ObjectReference) setPos l.pos, "emitOnly")
          emitAndGenPrint(nullCtx, LOAD_LOCAL(eqEqTempLocal), "emitOnly")
          emitAndGenPrint(nullCtx, CZJUMP(thenCtx.bb, elseCtx.bb, EQ, ObjectReference), "emitOnly")
          nullCtx.bb.close()
          /*nullCtx.bb.emitOnly(
          ...
          emitAndGenPrint(nonNullCtx, LOAD_LOCAL(eqEqTempLocal) setPos l.pos, "emitOnly")
          emitAndGenPrint(nonNullCtx, CALL_METHOD(Object_equals, Dynamic), "emitOnly")
          emitAndGenPrint(nonNullCtx, CZJUMP(thenCtx.bb, elseCtx.bb, NE, BOOL), "emitOnly")
          nonNullCtx.bb.close()
    ...
    class Context {
      ...
      def loadException(ctx: Context, exh: ExceptionHandler, pos: Position) = {
        ...
        emitAndGenPrint(ctx, LOAD_EXCEPTION(exh.loadExceptionClass) setPos pos, "emit", pos)
      ...
      def enterMethod(m: IMethod, d: DefDef): Context = {
        ...
        if(d.symbol.isConstructor)
          genStraight(ctx1, "##02", d.symbol.pos)
        for(params <- vparamss.reverse)
          for(param <- params.reverse) {
            genStraight(ctx1, "#1 " + param.symbol.fullName + " @", param.symbol.pos)
          }
        //if (!d.symbol.isStaticMember) 
          //genStraight(ctx1, "#1 this", d.symbol.pos)
      ...
      def Try(body: Context => Context,
        ...
        def emitFinalizer(ctx: Context): Context = if (!finalizer.isEmpty) {
          ...
          if(!ctx.bb.closed) {
            emitAndGenPrint(ctx, JUMP(ctx1.bb), "closeWithDefault")
            ctx.bb.close()
          }
          ...
          if (guardResult) {
            ...
            emitAndGenPrint(ctx2, LOAD_LOCAL(tmp), "emitDefault")
        ...
        if (settings.YdisableUnreachablePrevention || !outerCtx.bb.ignore) {
          if (finalizer != EmptyTree) {
            ...
            emitAndGenPrint(exhStartCtx, STORE_LOCAL(exception), "emitDefault")
            //exhStartCtx.bb.emit(STORE_LOCAL(exception))
            val exhEndCtx = genLoad(finalizer, exhStartCtx, UNIT)
            emitAndGenPrint(exhEndCtx, LOAD_LOCAL(exception), "emitDefault")
            //exhEndCtx.bb.emit(LOAD_LOCAL(exception))
            if(!exhEndCtx.bb.closed) {
              emitAndGenPrint(exhEndCtx, THROW(ThrowableClass), "closeWithDefault")
              exhEndCtx.bb.close()
            }
          ...
          for ((sym, kind, handler) <- handlers) {
            ...
            if(!exhEndCtx.bb.closed) {
              emitAndGenPrint(exhEndCtx, JUMP(normalExitCtx.bb), "closeWithDefault")
              exhEndCtx.bb.close()
            }
        ...
        if(!outerCtx.bb.closed) {
          emitAndGenPrint(outerCtx, JUMP(bodyCtx.bb), "closeWithDefault")
          outerCtx.bb.close()
        }
        ...
        if(!bodyEndCtx.bb.closed) {
          emitAndGenPrint(bodyEndCtx, JUMP(normalExitCtx.bb), "closeWithDefault")
          bodyEndCtx.bb.close()
        }
